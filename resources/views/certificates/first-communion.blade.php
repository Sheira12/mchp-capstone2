...
<div class="cert-body">
    ...
    <div class="cert-sub">Katibayan ng Unang Komunyon &nbsp;·&nbsp; Prima Communio</div>
    <div class="intro">Be it known to all that the following person has received<br>the Most Holy Eucharist for the first time in the Holy Catholic Church</div>
    <div class="recipient-block">
        <div class="recipient-name">{{ $certificate->parishioner->full_name }}</div>
        <div class="recipient-role">First Communicant</div>
    </div>
    <div class="det-grid">
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Date of First Communion</div><div class="det-val {{ $certificate->sacramentalRecord?->date_administered ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</div></div>
        ...
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Sponsor</div><div class="det-val {{ ($certificate->sacramentalRecord?->godparents[0] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->godparents[0] ?? 'Not recorded' }}</div></div>
