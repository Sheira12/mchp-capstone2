...
<div class="cert-content">
    ...
    <div class="cert-title-wrap">
        ...
        <span class="cert-title">Certificate of First Holy Communion</span>
        <span class="cert-sub">Katibayan ng Unang Komunyon &nbsp;·&nbsp; Prima Communio</span>
        <span class="gold-orn-sm">{!! $ornSm !!}</span>
        <span class="cert-intro">Be it known to all that the following person has received<br>the Most Holy Eucharist for the first time in the Holy Catholic Church</span>
    </div>
    <div class="recipient-wrap">
        <span class="recipient-name {{ $nameCls }}">{{ $certificate->parishioner->full_name }}</span>
        <span class="recipient-role">First Communicant</span>
    </div>
    <div class="details-wrap"><table class="details-tbl" cellpadding="0" cellspacing="0"><tr>
        <td class="det-left">
            <div class="det-item"><span class="det-lbl">Date of First Communion</span><span class="det-val {{ $certificate->sacramentalRecord?->date_administered ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</span></div>
        ...
        <td class="det-gap"></td>
        <td class="det-right">
            <div class="det-item"><span class="det-lbl">Sponsor</span><span class="det-val {{ ($certificate->sacramentalRecord?->godparents[0] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->godparents[0] ?? 'Not recorded' }}</span></div>
