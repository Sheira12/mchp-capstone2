...
@php $certificate = $item['certificate']; $qrBase64 = $item['qrBase64']; @endphp

<div class="cert-body" style="page-break-after: always;">
    ...
    <div class="intro">Be it known to all that the following person has been issued<br>this official certificate by the parish office</div>
    ...
    <div class="det-grid">
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Certificate Number</div><div class="det-val">{{ $certificate->certificate_number }}</div></div>
            <div class="det-item"><div class="det-lbl">Certificate Type</div><div class="det-val">{{ $certificate->getTypeLabel() }}</div></div>
            <div class="det-item"><div class="det-lbl">Parishioner</div><div class="det-val">{{ $certificate->parishioner->full_name }}</div></div>
        </div>
        <div class="det-col">
            ...
            <div class="det-item"><div class="det-lbl">Issued Date</div><div class="det-val">{{ $certificate->issued_date->format('F d, Y') }}</div></div>
            <div class="det-item"><div class="det-lbl">Purpose</div><div class="det-val {{ $certificate->purpose ? '' : 'na' }}">{{ $certificate->purpose ?? 'Official use' }}</div></div>
        </div>
    </div>
    ...
    {{-- Per-cert footer inside the page-break div --}}
    <div style="border-top:1pt solid rgba(212,175,55,0.6);padding-top:5pt;display:table;width:100%;margin-top:8pt;">
        <div style="display:table-cell;vertical-align:middle;width:28%;">
            <div style="font-family:Arial,sans-serif;font-size:6pt;font-weight:bold;letter-spacing:1pt;text-transform:uppercase;color:#D4AF37;">Certificate No.</div>
            <div style="font-family:'Courier New',monospace;font-size:6.5pt;color:#6A7A8A;margin-top:1pt;">{{ $certificate->certificate_number }}</div>
        </div>
        <div style="display:table-cell;vertical-align:middle;text-align:center;width:44%;">
            <div style="font-family:Arial,sans-serif;font-size:6pt;color:#9AAABB;line-height:1.6;">
                <strong style="color:#1F3A5F;font-size:6.5pt;">{{ $parish['name'] }}</strong><br>
                {{ $parish['address'] }}<br>
                Tel: {{ config('parish.phone') }} &nbsp;·&nbsp; {{ config('parish.email') }}
            </div>
        </div>
        <div style="display:table-cell;vertical-align:middle;text-align:right;width:28%;">
            @if(!empty($qrBase64))
            <img src="{{ $qrBase64 }}" style="width:40pt;height:40pt;display:block;margin-left:auto;" alt="QR">
            <div style="font-family:Arial,sans-serif;font-size:5.5pt;color:#9AAABB;margin-top:1.5pt;text-align:right;">Scan to verify</div>
            @endif
        </div>
    </div>
</div>
