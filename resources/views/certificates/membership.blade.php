...
$fullAddr = collect([$certificate->parishioner->address, $certificate->parishioner->barangay ? 'Brgy. '.$certificate->parishioner->barangay : null, $certificate->parishioner->city ?? 'Cabuyao', $certificate->parishioner->province ?? 'Laguna'])->filter()->implode(', ');
@endphp
...
<div class="cert-content">
    ...
    <div class="cert-title-wrap">
        ...
        <span class="cert-title">Certificate of Parish Membership</span>
        <span class="cert-sub">Katibayan ng Pagiging Miyembro ng Parokya</span>
        <span class="gold-orn-sm">{!! $ornSm !!}</span>
        <span class="cert-intro">Be it known to all that the following person is a duly registered<br>and active member of this Holy Catholic Parish</span>
    </div>
    <div class="recipient-wrap">
        <span class="recipient-name {{ $nameCls }}">{{ $certificate->parishioner->full_name }}</span>
        <span class="recipient-role">Registered Parishioner &nbsp;·&nbsp; Active Member</span>
    </div>
    <div class="details-wrap"><table class="details-tbl" cellpadding="0" cellspacing="0"><tr>
        <td class="det-left">
            ...
            <div class="det-item"><span class="det-lbl">Contact Number</span><span class="det-val {{ $certificate->parishioner->contact_number ? '' : 'na' }}">{{ $certificate->parishioner->contact_number ?? 'Not recorded' }}</span></div>
            <div class="det-item"><span class="det-lbl">Gender</span><span class="det-val">{{ ucfirst($certificate->parishioner->gender ?? 'Not recorded') }}</span></div>
        </td>
        <td class="det-gap"></td>
        <td class="det-right">
            <div class="det-item"><span class="det-lbl">Address</span><span class="det-val {{ $certificate->parishioner->address ? '' : 'na' }}">{{ $fullAddr ?: 'Not recorded' }}</span></div>
            <div class="det-item"><span class="det-lbl">Registered Since</span><span class="det-val">{{ $certificate->parishioner->created_at->format('F d, Y') }}</span></div>
