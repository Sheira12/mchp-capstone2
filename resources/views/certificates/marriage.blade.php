...
<div class="cert-body">
    ...
    <div class="cert-sub">Katibayan ng Kasal &nbsp;·&nbsp; Sacramentum Matrimonii</div>
    <div class="intro">Be it known to all that the following persons have been united<br>in the Sacrament of Holy Matrimony in the Holy Catholic Church</div>
    <div class="recipient-block">
        <div class="recipient-name">{{ $certificate->parishioner->full_name }}</div>
        @if($certificate->sacramentalRecord?->spouseParishioner)
        <div style="font-size:7pt;color:#D4AF37;letter-spacing:2pt;text-transform:uppercase;margin:2pt 0;">and</div>
        <div class="recipient-name" style="font-size:18pt;">{{ $certificate->sacramentalRecord->spouseParishioner->full_name }}</div>
        @endif
        <div class="recipient-role">United in Holy Matrimony</div>
    </div>
    <div class="det-grid">
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Date of Marriage</div><div class="det-val {{ $certificate->sacramentalRecord?->date_administered ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</div></div>
            ...
            <div class="det-item"><div class="det-lbl">Witness 1</div><div class="det-val {{ ($certificate->sacramentalRecord?->witnesses[0] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->witnesses[0] ?? 'Not recorded' }}</div></div>
        </div>
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Principal Sponsor (Ninong)</div><div class="det-val {{ ($certificate->sacramentalRecord?->sponsors[0] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->sponsors[0] ?? 'Not recorded' }}</div></div>
            <div class="det-item"><div class="det-lbl">Principal Sponsor (Ninang)</div><div class="det-val {{ ($certificate->sacramentalRecord?->sponsors[1] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->sponsors[1] ?? 'Not recorded' }}</div></div>
            <div class="det-item"><div class="det-lbl">Witness 2</div><div class="det-val {{ ($certificate->sacramentalRecord?->witnesses[1] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->witnesses[1] ?? 'Not recorded' }}</div></div>
            <div class="det-item"><div class="det-lbl">Register / Page / Line</div><div class="det-val">{{ $certificate->sacramentalRecord?->register_number ?? '—' }} / {{ $certificate->sacramentalRecord?->page_number ?? '—' }} / {{ $certificate->sacramentalRecord?->line_number ?? '—' }}</div></div>
        </div>
    </div>
