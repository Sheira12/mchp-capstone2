...
<div class="page">
    <div class="border-outer">
        <div class="border-inner">
            ...
            <div class="cert-subtitle">Katibayan ng Kasal</div>
            <p class="body-text">
                This is to certify that <span class="field">{{ $certificate->parishioner->full_name }}</span>
                and <span class="field">{{ $certificate->sacramentalRecord?->spouseParishioner?->full_name ?? '___________________' }}</span>
                were united in the Sacrament of <strong>Holy Matrimony</strong> in this parish on
            ...
            <table class="details-table">
                <tr>
                    ...
                    <td class="value">{{ $certificate->parishioner->full_name }}</td>
                    <td class="label">Bride:</td>
                    <td class="value">{{ $certificate->sacramentalRecord?->spouseParishioner?->full_name ?? '—' }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class="label">Principal Sponsor (Ninong):</td>
                    <td class="value">{{ $certificate->sacramentalRecord?->sponsors[0] ?? '—' }}</td>
                    <td class="label">Principal Sponsor (Ninang):</td>
                    <td class="value">{{ $certificate->sacramentalRecord?->sponsors[1] ?? '—' }}</td>
                </tr>
                <tr>
                    <td class="label">Witness 1:</td>
                    <td class="value">{{ $certificate->sacramentalRecord?->witnesses[0] ?? '—' }}</td>
                    <td class="label">Witness 2:</td>
                    <td class="value">{{ $certificate->sacramentalRecord?->witnesses[1] ?? '—' }}</td>
                </tr>
                <tr>
