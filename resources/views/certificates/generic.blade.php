...
<div class="cert-content">
    ...
    <div class="cert-title-wrap">
        ...
        <span class="cert-title">{{ $certificate->getTypeLabel() }}</span>
        <span class="cert-sub">Official Parish Certificate &nbsp;·&nbsp; Certificatum Officiale</span>
        <span class="gold-orn-sm">{!! $ornSm !!}</span>
        <span class="cert-intro">Be it known to all that the following person is a registered parishioner<br>of this Holy Catholic Parish and the information herein is true and correct</span>
    </div>
    <div class="recipient-wrap">
        <span class="recipient-name {{ $nameCls }}">{{ $certificate->parishioner->full_name }}</span>
        <span class="recipient-role">Registered Parishioner</span>
    </div>
    <div class="details-wrap"><table class="details-tbl" cellpadding="0" cellspacing="0"><tr>
        <td class="det-left">
            @if($certificate->sacramentalRecord)
            <div class="det-item"><span class="det-lbl">Date</span><span class="det-val">{{ $certificate->sacramentalRecord->date_administered?->format('F d, Y') ?? '—' }}</span></div>
            <div class="det-item"><span class="det-lbl">Celebrant</span><span class="det-val {{ $certificate->sacramentalRecord->celebrant ? '' : 'na' }}">{{ $certificate->sacramentalRecord->celebrant ?? 'Not recorded' }}</span></div>
