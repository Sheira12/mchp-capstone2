...
<div class="cert-body">
    ...
    <div class="cert-sub">Katibayan ng Walang Hadlang sa Kasal &nbsp;·&nbsp; Nihil Obstat</div>
    <div class="intro">Be it known to all that the following person is free to marry<br>in the Holy Catholic Church with no known canonical impediment</div>
    <div class="recipient-block">
        <div class="recipient-name">{{ $certificate->parishioner->full_name }}</div>
        <div class="recipient-role">Free to Marry in the Catholic Church</div>
    </div>
    <div class="det-grid">
        <div class="det-col">
            ...
            <div class="det-item"><div class="det-lbl">Address</div><div class="det-val {{ $certificate->parishioner->address ? '' : 'na' }}">{{ $certificate->parishioner->address ?? 'Not recorded' }}@if($certificate->parishioner->barangay), Brgy. {{ $certificate->parishioner->barangay }}@endif, {{ $certificate->parishioner->city ?? 'Cabuyao' }}</div></div>
        </div>
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Baptized At</div><div class="det-val">{{ $certificate->parishioner->baptismRecord?->venue ?? $parish['name'] }}</div></div>
            <div class="det-item"><div class="det-lbl">Date of Baptism</div><div class="det-val {{ $certificate->parishioner->baptismRecord?->date_administered ? '' : 'na' }}">{{ $certificate->parishioner->baptismRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</div></div>
            <div class="det-item"><div class="det-lbl">Parish</div><div class="det-val">{{ $parish['name'] }}</div></div>
        </div>
    </div>
    <div class="issuance">Issued this <b>{{ $certificate->issued_date->format('jS') }}</b> day of <b>{{ $certificate->issued_date->format('F Y') }}</b> for the purpose of <b>{{ $certificate->purpose ?? 'marriage preparation' }}</b>.</div>
    <div class="sig-row">
