...
$cityAddr = collect([$certificate->parishioner->address, $certificate->parishioner->barangay ? 'Brgy. '.$certificate->parishioner->barangay : null, $certificate->parishioner->city ?? 'Cabuyao'])->filter()->implode(', ');
@endphp
...
<div class="cert-content">
    ...
    <div class="cert-title-wrap">
        ...
        <span class="cert-title">Certificate of No Impediment</span>
        <span class="cert-sub">Katibayan ng Walang Hadlang &nbsp;·&nbsp; Nihil Obstat</span>
        <span class="gold-orn-sm">{!! $ornSm !!}</span>
        <span class="cert-intro">Be it known to all that the following person is free to marry<br>in the Holy Catholic Church with no known canonical impediment</span>
    </div>
    <div class="recipient-wrap">
        <span class="recipient-name {{ $nameCls }}">{{ $certificate->parishioner->full_name }}</span>
        <span class="recipient-role">Free to Marry in the Holy Catholic Church</span>
    </div>
    <div class="details-wrap"><table class="details-tbl" cellpadding="0" cellspacing="0"><tr>
        <td class="det-left">
            ...
            <div class="det-item"><span class="det-lbl">Civil Status</span><span class="det-val">{{ ucfirst($certificate->parishioner->civil_status ?? 'Not recorded') }}</span></div>
            <div class="det-item"><span class="det-lbl">Address</span><span class="det-val {{ $certificate->parishioner->address ? '' : 'na' }}">{{ $cityAddr ?: 'Not recorded' }}</span></div>
            <div class="det-item"><span class="det-lbl">Parish</span><span class="det-val">{{ $parish['name'] }}</span></div>
        </td>
        <td class="det-gap"></td>
        <td class="det-right">
            <div class="det-item"><span class="det-lbl">Baptized At</span><span class="det-val">{{ $certificate->parishioner->baptismRecord?->venue ?? $parish['name'] }}</span></div>
            <div class="det-item"><span class="det-lbl">Date of Baptism</span><span class="det-val {{ $certificate->parishioner->baptismRecord?->date_administered ? '' : 'na' }}">{{ $certificate->parishioner->baptismRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</span></div>
            <div class="det-item"><span class="det-lbl">No Previous Marriage</span><span class="det-val">Confirmed by Parish Records</span></div>
            <div class="det-item"><span class="det-lbl">Canonical Status</span><span class="det-val" style="color:#1a7a3c;">Free to Marry ✓</span></div>
        </td>
    </tr></table></div>
    <div class="issuance-wrap">Issued this <b>{{ $certificate->issued_date->format('jS') }}</b> day of <b>{{ $certificate->issued_date->format('F Y') }}</b>, at <b>Mary Help of Christians Parish</b>, Cabuyao, Laguna, for the purpose of <b>{{ $certificate->purpose ?? 'marriage preparation' }}</b>.</div>
