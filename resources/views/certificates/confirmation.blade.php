...
<div class="cert-body">
    ...
    <div class="cert-sub">Katibayan ng Kumpil &nbsp;·&nbsp; Sacramentum Confirmationis</div>
    <div class="intro">Be it known to all that the following person has received<br>the Sacrament of Confirmation in the Holy Catholic Church</div>
    <div class="recipient-block">
        <div class="recipient-name">{{ $certificate->parishioner->full_name }}</div>
        <div class="recipient-role">Recipient of Confirmation</div>
    </div>
    <div class="det-grid">
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Date of Confirmation</div><div class="det-val {{ $certificate->sacramentalRecord?->date_administered ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->date_administered?->format('F d, Y') ?? 'Not recorded' }}</div></div>
            ...
            <div class="det-item"><div class="det-lbl">Celebrant / Bishop</div><div class="det-val {{ $certificate->sacramentalRecord?->celebrant ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->celebrant ?? 'Not recorded' }}</div></div>
            <div class="det-item"><div class="det-lbl">Venue</div><div class="det-val">{{ $certificate->sacramentalRecord?->venue ?? $parish['name'] }}</div></div>
        </div>
        <div class="det-col">
            <div class="det-item"><div class="det-lbl">Sponsor (Ninong/Ninang)</div><div class="det-val {{ ($certificate->sacramentalRecord?->sponsors[0] ?? $certificate->sacramentalRecord?->godparents[0] ?? null) ? '' : 'na' }}">{{ $certificate->sacramentalRecord?->sponsors[0] ?? ($certificate->sacramentalRecord?->godparents[0] ?? 'Not recorded') }}</div></div>
