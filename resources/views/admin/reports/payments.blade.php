...
<div class="py-6 space-y-5">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 no-print">
        <form method="GET" class="space-y-3">
            ...
            {{-- Year + custom date range + actions all on one row --}}
            <div class="flex flex-wrap gap-2 items-end">
                <div>
                    <label class="form-label text-xs">Year</label>
                    <select name="year" class="form-select text-sm" style="width:80px;">
                ...
                <button type="submit" class="btn-primary text-sm px-4 py-2 self-end">Apply</button>
                <button type="button" onclick="window.print()"
                        class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg flex items-center gap-2 transition self-end">
                ...
                <a href="{{ route('admin.reports.payments', array_merge(request()->query(), ['export' => 'pdf', 'quarter' => $data['quarter'], 'year' => $data['year']])) }}"
                   class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg flex items-center gap-2 transition self-end">
                ...
                <a href="{{ route('admin.reports.payments', array_merge(request()->query(), ['export' => 'excel', 'quarter' => $data['quarter'], 'year' => $data['year']])) }}"
                   class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg flex items-center gap-2 transition self-end">
                ...
                <a href="{{ route('admin.reports.index') }}" class="btn-secondary text-sm self-end">← All Reports</a>
            </div>
        </form>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 print-section">
            ...
            <table class="w-full text-sm">
                <thead><tr class="bg-green-700 text-white text-left">
                    <th class="px-3 py-2 whitespace-nowrap">Payment Method</th>
                    <th class="px-3 py-2 text-right whitespace-nowrap">Transactions</th>
                    <th class="px-3 py-2 text-right whitespace-nowrap">Total (PHP)</th>
