...
@section('page-title', 'Edit Ledger Entry')

@section('content')
<div class="py-6 max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.ledger.update', $ledger) }}" class="space-y-5">
            @csrf @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-3 mt-1">
                    <label class="cursor-pointer">
                        <input type="radio" name="type" value="credit" class="sr-only peer"
                               {{ old('type', $ledger->type) === 'credit' ? 'checked' : '' }}
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="type" value="debit" class="sr-only peer"
                               {{ old('type', $ledger->type) === 'debit' ? 'checked' : '' }}
            ...
            <div>
                ...
                <select name="category" id="category-select" class="form-select w-full @error('category') border-red-400 @enderror" required>
                    ...
                    <option value="{{ $key }}" data-type="credit" {{ old('category', $ledger->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                    @foreach(\App\Models\LedgerEntry::DEBIT_CATEGORIES as $key => $label)
                    <option value="{{ $key }}" data-type="debit" {{ old('category', $ledger->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="description" value="{{ old('description', $ledger->description) }}" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="amount" value="{{ old('amount', $ledger->amount) }}" required min="0.01" step="0.01"
                ...
                <div>
                    ...
                    <input type="date" name="entry_date" value="{{ old('entry_date', $ledger->entry_date->toDateString()) }}" required
            ...
            <div>
                ...
                <input type="text" name="reference_number" value="{{ old('reference_number', $ledger->reference_number) }}"
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="form-input w-full">{{ old('notes', $ledger->notes) }}</textarea>
            </div>
...
function updateCategories(type) {
    ...
    // Try to keep current value if same type
    if (sel.querySelector(`option[value="${current}"][data-type="${type}"]`)) {
        sel.value = current;
    } else {
        sel.value = '';
    }
