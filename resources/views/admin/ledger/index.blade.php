...
@push('styles')
<style>
.lcard { background:#fff; border:1px solid #e8edf5; border-radius:1rem; padding:1rem; transition:box-shadow 0.15s; }
.lcard:hover { box-shadow:0 4px 16px rgba(0,0,0,0.07); }
.lcard-top { display:flex; align-items:flex-start; justify-content:space-between; gap:0.5rem; margin-bottom:0.5rem; }
.lcard-desc { font-weight:600; font-size:0.9rem; color:#0f172a; }
.lcard-meta { font-size:0.75rem; color:#64748b; margin-top:2px; }
.lcard-amount { font-weight:800; font-size:1.1rem; white-space:nowrap; }
.lcard-footer { display:flex; align-items:center; justify-content:space-between; gap:0.5rem; margin-top:0.625rem; padding-top:0.625rem; border-top:1px solid #f1f5f9; flex-wrap:wrap; }
</style>
@endpush
...
<div class="py-6 pb-20 space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-green-50 border border-green-200 rounded-xl p-5">
            <p class="text-xs font-bold text-green-600 uppercase tracking-wide mb-1">Total Income</p>
            <p class="text-2xl font-bold text-green-700">₱{{ number_format($overallCredit, 2) }}</p>
        </div>
        <div class="bg-red-50 border border-red-200 rounded-xl p-5">
            <p class="text-xs font-bold text-red-600 uppercase tracking-wide mb-1">Total Expenses</p>
            <p class="text-2xl font-bold text-red-700">₱{{ number_format($overallDebit, 2) }}</p>
        </div>
        <div class="{{ ($overallCredit-$overallDebit)>=0 ? 'bg-blue-50 border-blue-200' : 'bg-red-50 border-red-200' }} border rounded-xl p-5">
            ...
            <p class="text-2xl font-bold {{ ($overallCredit-$overallDebit)>=0 ? 'text-blue-700' : 'text-red-700' }}">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
        <form method="GET" data-live-search data-target="#ledger-list" class="flex flex-wrap gap-3 items-end">
            <div>
    ...
    {{-- ── MOBILE CARDS ── --}}
    <div id="ledger-list" class="space-y-3 lg:hidden">
        @forelse($entries as $entry)
        <div class="lcard">
            <div class="lcard-top">
                <div class="flex-1 min-w-0">
                    <p class="lcard-desc truncate">{{ $entry->description }}</p>
                    <p class="lcard-meta">{{ $entry->entry_date->format('M d, Y') }} &middot; {{ $entry->category }}</p>
                    @if($entry->reference_number)
                    <p class="lcard-meta font-mono">{{ $entry->reference_number }}</p>
                    @endif
                </div>
                <p class="lcard-amount {{ $entry->type==='credit' ? 'text-green-700' : 'text-red-700' }}">
            ...
            <div class="lcard-footer">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <form method="POST" action="{{ route('admin.ledger.destroy', $entry) }}" onsubmit="return confirm('Delete this entry?')" class="inline">
                        @csrf @method('DELETE')
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-10 text-center text-gray-400">
            No entries found. <a href="{{ route('admin.ledger.create') }}" class="text-blue-600 hover:underline">Add the first entry →</a>
        </div>
        @endforelse
        @if($entries->hasPages())
        <div class="bg-white rounded-xl border border-gray-100 px-4 py-3">{{ $entries->links() }}</div>
        @endif
    </div>

    {{-- ── DESKTOP TABLE ── --}}
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hidden lg:block mb-10">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{{ $entry->entry_date->format('M d, Y') }}</td>
                    <td class="px-4 py-3">
                        @if($entry->type==='credit')
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>Income
                        </span>
                        @else
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-700">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>Expense
                        </span>
                        @endif
                    </td>
